...
นอกจากสร้าง hint ใหม่ `/research-stock` ยังเช็คย้อนกลับด้วยว่าหุ้นที่กำลัง research ตัวนี้**เกี่ยวข้อง/อาจได้รับผลกระทบ**จาก hint ที่มีอยู่แล้วหรือไม่ (ขั้น 2.6 — ไม่ว่า hint นั้นจะเจอวันนี้หรือก่อนหน้า) ถ้าใช่ ต้อง **factor เข้ากับการวิเคราะห์จริง ไม่ใช่แค่แปะหมายเหตุ**: analyst ปรับ `risk_level`/verdict ถ้ากระทบจริงจัง + เพิ่มหัวข้อ "จาก [hint] ต้องระวังอะไร", theorist ใส่ใน `risks`/bear scenario **และ** ปรับ `entry_plan` (ลด allocation ไม้ลึก / เพิ่มเงื่อนไขใน trigger / เพิ่มสัญญาณใน invalidation_md) พร้อมลิงก์ `/insights/<slug>` ทุกจุดที่อ้างถึง
