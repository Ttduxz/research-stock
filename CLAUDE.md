...
- hint ใหม่: `hint-analyst` เขียนมาเองตั้งแต่แรก (`ingest-hint.mjs` ตรวจ) · hint เก่า: `npm run hints:illustrate -- export <DIR> [--slug X]` → agent `hint-illustrator` (opus, Read/Write เท่านั้น ไม่ค้นเว็บ) เขียน `<slug>.illustrated.json` → `npm run hints:illustrate -- apply <file>` (content_md แก้ได้แค่แทรก marker — สคริปต์เทียบว่าตัด marker แล้วต้องเท่าต้นฉบับ)
