...
- ถ้า orchestrator แนบสรุป **hint** ที่เกี่ยวข้อง (รายงานประเด็นเชิงระบบ/อุตสาหกรรมที่แยกวิเคราะห์ไว้ต่างหาก) มาให้ **ต้องเอาไป factor เข้ากับทั้งทฤษฎีและ `entry_plan` จริง ไม่ใช่แค่พูดถึงเฉยๆ**:
  - ใส่เป็นหนึ่งใน `risks` ของทฤษฎีที่เกี่ยวข้อง ระบุ exposure เฉพาะของหุ้นตัวนี้ พร้อมลิงก์ `[ชื่อรายงาน](/insights/<slug>)`
  - ถ้าเข้าเค้าจริงๆ ให้สะท้อนลงใน rationale ของ **bear scenario** ด้วย (ไม่ใช่แค่ risks list เฉยๆ)
  - **`entry_plan` ต้องคิดเรื่องนี้เป็นปัจจัยความเสี่ยงจริงตอนวางแผนเข้าซื้อ**: ถ้า exposure สูง ให้พิจารณาลด `allocation` ของไม้ที่ลึกกว่า (2-3) หรือใส่เงื่อนไขเพิ่มใน `trigger` ว่าต้องเห็นสัญญาณจาก hint นั้นคลี่คลาย/ไม่แย่ลงก่อนค่อยเข้าไม้ถัดไป และใส่สัญญาณจาก hint (เช่น ตัวชี้วัดที่ hint ระบุว่าต้องจับตา) ไว้เป็นหนึ่งในเงื่อนไขของ `invalidation_md` ด้วย — ไม่ใช่แผนเข้าซื้อเดียวกับที่ไม่มี hint อยู่เลย
