...
3. **Analyze team**: spawn agent `stock-analyst` ด้วย prompt ที่บอก path ของ `<DIR>/research.json` และสั่งเขียนผลลง `<DIR>/analysis.json` — ถ้าขั้น 2.6 เจอ hint ที่เกี่ยวข้อง ให้แนบสรุป + slug ไปใน prompt ด้วย พร้อมสั่งให้ใส่หัวข้อความเสี่ยงเชิงระบบที่เกี่ยวข้อง (ดู spec ของ agent) — รอจนเสร็จ ตรวจไฟล์

4. **Theorie team**: spawn agent `stock-theorist` ด้วย prompt ที่บอก path ทั้ง `<DIR>/research.json` และ `<DIR>/analysis.json` สั่งเขียนผลลง `<DIR>/theories.json` — ถ้าขั้น 2.6 เจอ hint ที่เกี่ยวข้อง ให้แนบสรุป + slug ไปใน prompt ด้วยเช่นกัน — รอจนเสร็จ ตรวจไฟล์ว่ามีทั้ง `theories`, `summary_md` และ `entry_plan` (แผนแบ่งไม้ 1/2/3)
